...
export default function RegisterForm({ code }: { code: string }) {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-[var(--text-muted)] mb-1">Referral Code (Optional)</label>
        <input
          name="refCode"
          type="text"
          defaultValue={code}
          readOnly={!!code}
          className={`w-full bg-[var(--surface-900)] border border-[var(--surface-600)] rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[var(--gold-500)] ${!!code ? "opacity-50 cursor-not-allowed" : ""}`}
          placeholder="xyz-abc..."
        />
      </div>
